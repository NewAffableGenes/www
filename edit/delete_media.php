...
// The media to delete
$media = $return['media'];
// The type of object to delete the media from
$type = $return['type'];
// The id of object to delete the media from
$id = $return[$type];

if ((strlen($errorTitle) == 0) && (($media == null) || ($type == null) || ($id == null))) {
    $errorTitle = "Selection Error: delete media";
    $errorMessage = "You have not selected the right elements from this tree";
    $errorRedirect = "/tree/tree.php";
}

if (strlen($errorTitle) == 0) {
    delete_entry($mysqli, 'media', $media, $id);
