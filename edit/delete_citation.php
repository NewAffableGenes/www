...
// The type of object to delete the citation from
$type = $return['type'];
// The id of object to delete the citation from
$id = $return[$type];

if ((strlen($errorTitle) == 0) && (($citation == null) || ($type == null) || ($id == null))) {
    $errorTitle = "Selection Error: delete citation";
    $errorMessage = "You have not selected the right elements from this tree";
    $errorRedirect = "/tree/tree.php";
}

if (strlen($errorTitle) == 0) {
    delete_entry($mysqli, "citation", $citation);
