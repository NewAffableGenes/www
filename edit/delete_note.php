...
// The note to delete
$note = $return['note'];
// The type of object to delete the note from
$type = $return['type'];
// The id of object to delete the note from
$id = $return[$type];

if ((strlen($errorTitle) == 0) && (($note == null) || ($type == null) || ($id == null))) {
    $errorTitle = "Selection Error: delete note";
    $errorMessage = "You have not selected the right elements from this tree";
    $errorRedirect = "/tree/tree.php";
}

if (strlen($errorTitle) == 0) {
    delete_entry($mysqli, $type, $note, $id);
